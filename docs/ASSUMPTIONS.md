...
**Terms, Risk disclosure, and Privacy** (`terms.html`, `risk-disclosure.html`, `privacy.html`) are now real pages with substantive, product-accurate draft text — not placeholders — covering what the app actually does (Song Ownership, wallet-funded bonding-curve Backing, manual withdrawal fulfillment, what data is collected and by whom). **These are drafts written by an agent, not a lawyer, and have not been reviewed by counsel.** Per the founder's earlier instruction to own legal/licensing ("we'll deal with all legal issues and licenses on our end"), these need a legal review pass before Nextup actually opens to the public — flagged here rather than silently treated as final.
