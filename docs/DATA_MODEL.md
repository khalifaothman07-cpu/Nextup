...
- **All five are `SECURITY DEFINER`, take no `user_id` argument, read `auth.uid()` themselves, and refuse any caller without the `admin` role** — the deliberate opposite of the `service_role`-only pattern used by `open_position`/`request_withdrawal`. `docs/SECURITY.md` explains why the two shapes need different grants.
