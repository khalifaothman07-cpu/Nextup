...
## Double-entry ledger and treasury (Cycle 16)

`wallets.balance_cents` is still the operational balance — nothing consults the ledger to decide whether a user can afford something. The ledger is an **independent second record** whose job is to say whether those balances are backed. Reconciliation between the two is the whole value: two records that agree prove something, one record proves nothing.

- **`ledger_accounts`** — `id, kind, owner_user_id, label, created_at`. Five kinds: `customer_wallet` and `customer_escrow` (per user, `owner_user_id` set), `company_operating`, `company_revenue`, `settlement` (company-side, `owner_user_id` null). Partial unique indexes enforce one account per user per customer kind and one per company kind, so commingling company and customer money is a schema error rather than a policy someone has to remember.
- **`ledger_transactions`** — `id, kind, reference_table, reference_id, memo, created_at`. One row per movement of value, pointing back at the row that caused it.
- **`ledger_entries`** — `id, transaction_id, account_id, amount_cents, created_at`. Signed; never updated or deleted in normal operation.
- **`private.assert_ledger_balanced()`** — a **`DEFERRABLE INITIALLY DEFERRED` constraint trigger** on `ledger_entries`. At commit it requires every transaction to have at least two entries summing to exactly zero, and refuses the commit otherwise. Deferred because a balanced transaction is only balanced once all its legs are in; enforcing per-statement would reject the first leg of every correct transaction. `set search_path = ''` with `public.ledger_entries` written out in full, after the advisor flagged the mutable path.
- **Sign convention: assets positive, liabilities negative.** A customer's wallet balance is money the company _owes_, so `customer_wallet` entries are negative and company holdings are positive. `obligations = -sum(customer entries)`, `holdings = sum(company entries)`, `coverage = holdings - obligations`. Negative coverage means unbacked liability. Getting this backwards makes every figure in the treasury panel wrong in a plausible-looking way, which is why it is written down here and asserted by probe rather than left to be re-derived.
- **`private.post_ledger(kind, reference_table, reference_id, memo, entries)`** — the single writer. Creates the transaction and its entries together, auto-creating a user's `customer_wallet` account on first use.
- **Wired by triggers on the tables that already move money**, not by callers: `wallet_deposits`, `positions` (insert _and_ update, so opening and closing both post), `withdrawal_requests`, `song_ownership`. Same reasoning as the rate limiters — caller-side posting is skipped the moment anything writes the table by another path, and every one of these tables has more than one.
- **RLS**: `SELECT`-only policies for `authenticated`, scoped to accounts the caller owns (entries and transactions join through `ledger_accounts.owner_user_id`). **No `INSERT`/`UPDATE`/`DELETE` policy exists on any of the three tables**, so the only writer is `private.post_ledger` running as the table owner. The `ledger_position` aggregate view is `service_role`-only.
- **`admin_treasury_position()`** — obligations, holdings, coverage, and per-kind totals. **`admin_reconcile_wallets()`** — compares every `wallets.balance_cents` against the ledger's view of that wallet and returns only the rows that disagree, with the drift. Both are the `auth.uid()`-deriving, self-role-checking shape (see `docs/SECURITY.md`), so `authenticated` correctly holds `EXECUTE`.
- **A `numeric`/`bigint` trap worth remembering**: `sum()` over `bigint` returns `numeric`, so the first `admin_treasury_position()` signature did not match its own body. `CREATE OR REPLACE VIEW` cannot change a column's type and does not say so clearly — the view had to be dropped and recreated.

**What the ledger found, which is the point of having built it**: `open_position` debits the wallet and the stake goes nowhere; `close_position` credits proceeds that come from nowhere. The bonding curve has no reserve behind it. See `docs/ARCHITECTURE.md` → Architectural risks.
...
The master prompt's full entity list (`SupportTier`, `SupportSubscription`, `SupportPayment`, `Benefit`, `BenefitEntitlement`, `ArtistPost`, `Comment`, `Reaction`, `Poll`, `PollOption`, `PollVote`, `Product`, `ProductVariant`, `InventoryRecord`, `Order`, `OrderItem`, `PaymentRecord`, `RefundRecord`, `CommunityMembership`, `Notification`, `NotificationPreference`, `Scene`, `Genre`, `ArtistScene`, `SearchRecord`, `DiscoveryEvent`, `Report`, `ModerationAction`, `AuditLog`, `ArtistLead`, `ArtistLeadNote`, `ArtistLeadActivity`, `ArtistMetricSnapshot`, `ArtistVerification`) does not exist yet — though `ArtistFollow` and `ArtistMomentumScore` are now covered by `artist_follows`/`artist_momentum_daily` above (`ArtistSave` was folded into follow rather than built as a separate near-duplicate concept), and `LedgerAccount`/`LedgerTransaction`/`LedgerEntry` by the ledger section above. None of the rest is faked or stubbed in the UI.
